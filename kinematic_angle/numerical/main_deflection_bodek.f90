program main_deflection
  ...
  type(gpf) :: gp
  real(dp), dimension(n_pts) :: energy, angle, d_angle
  ...
  do j = 1, n_pts
    ...
    !write (*,*) E, integral
    energy(j) = E
    angle(j) = integral(2)/integral(1)
    d_angle(j) = sqrt(integral(3)/integral(1) - angle(j)**2)
    !write (*,*) E, &
    !  integral(2)/integral(1), &
    !  sqrt(integral(3)/integral(1) - (integral(2)/integral(1))**2), &
    write (*,*) E, angle(j), d_angle(j), ifail, 0.7_dp/(E/1e3_dp)**0.6_dp
  end do

  call gp%options('set log x')
  call gp%options('set xlabel "E, GeV"')
  call gp%options('set ylabel "θ, °"')
  call gp%plot(energy, angle, 'w l', '', &
   energy, angle - d_angle, 'w l dashtype 2', '', &
   energy, angle + d_angle, 'w l dashtype 2', '')
contains
  ...
  function dsigma_neutrino(E, y, Q2) result(dsigma)
    ...
    dsigma = GF**2/(4*pi)*(MW**2/(Q2 + MW**2))**2 &
      ...
      /y*(F2*(1 + (1 - y)**2 - y**2) + xF3*(1 - (1 - y)**2) + 2*y**2*xF1)
