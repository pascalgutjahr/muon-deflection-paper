...
\abstract{
  ...
  This is on the order of magnitude of the directional resolution of present neutrino detectors. Furthermore, comparisons with the simulation tools MUSIC and \pascaledit{\textsc{Geant4}} as well as two different muon deflection measurements are performed.
  \keywords{Neutrino Astronomy -- Neutrino Source Search -- Angular Resolution -- \pascalreplace{Muon Tomography}{Muography}}
%
% \PACS{
...
The directional reconstruction of muons is an essential task for \pascalreplace{muon tomography}{muography} or large scale neutrino detectors such as IceCube 
...
MUSIC \pascaledit{\cite{MUSIC, comparison_MUSIC_GEANT4_2009}} and \pascaledit{\textsc{Geant4}} \pascaledit{\cite{GEANT4_standard, GEANT4}} and data from two experiments in section~\ref{sec:accum_defl}. The findings of this study
...
\pascaledit{Typically, energy cuts of $v_{\mathrm{cut}} \leq \num{0.05}$ and 
$e_{\mathrm{cut}} = \SI{500}{\mega\electronvolt}$ are chosen which enable 
accurate propagations at low runtimes.}
The 
...
\pascaldelete{\cite{phd_soedingrekso}}.
...
Furthermore, there are parametrizations for stochastic deflections given in \pascaledit{\textsc{Geant4}} \pascaledit{\cite{GEANT4_standard, GEANT4}} 
...
\pascaledit{(MSM)} \cite{moliere_scattering} and the Gaussian approximation by 
Highland \pascaledit{(MSH) \cite{HIGHLAND_1975}}
can be chosen. 
\pascaledit{MSM results as a summation of elastic scatterings of one particle 
...
\begin{figure*}
    ...
    Two simulations 
    are done to check both multiple scattering methods Molière \pascaledit{(MSM)} 
    and Highland \pascaledit{(MSH)}.  
    The total distribution is presented only for all stochastic processes including \pascalreplace{Molière's}{MSM}. 
    \pascaledit{Since multiple scattering describes the deflection along a 
    continuous energy loss, a single deflection occurs similar to a stochastic 
    interaction.}
...
\begin{table*}
    ...
    \pascalreplace{Molière's}{MSM} with the upper and lower limits of the $\SI{95}{\percent}$ 
    central intervals. The largest median deflection is caused by photonuclear interaction.}
    \begin{tabular}{ccccccc}
        \toprule 
        Brems & Nuclint & Epair & Ioniz & \pascalreplace{Molière}{MSM} & \pascalreplace{Highland}{MSH} & Total  \vspace{6pt} \\
        $\theta\,/\,\num{e-5}\,\pascaledit{\text{deg}}$ & $\theta\,/\,\num{e-5}\,\pascaledit{\text{deg}}$ & $\theta\,/\,\num{e-5}\,\pascaledit{\text{deg}}$ & $\theta\,/\,\num{e-5}\,\pascaledit{\text{deg}}$ & $\theta\,/\,\num{e-5}\,\pascaledit{\text{deg}}$ & $\theta\,/\,\num{e-5}\,\pascaledit{\text{deg}}$ & $\theta\,/\,\num{e-5}\,\pascaledit{\text{deg}}$\\
        \midrule 
...
\subsection{Comparison with MUSIC and \pascaledit{\textsc{Geant4}}}
First, the deflections in PROPOSAL are compared to 
the tools MUSIC \pascaldelete{\cite{MUSIC,comparison_MUSIC_GEANT4_2009}} and \pascaledit{\textsc{Geant4}} \pascaldelete{\cite{GEANT4}}.
...
\pascaledit{\textsc{Geant4}} is another common toolkit to simulate the passage of particles through 
...
made for simulations in particle detectors \pascaldelete{but the computing is 
intensive} \pascaledit{\cite{GEANT4_standard, GEANT4}}. 
...
largest displacements are exhibited by \pascaledit{\textsc{Geant4}} and PROPOSAL with Molière scattering, which 
...
The largest average deflections are obtained in \pascaledit{\textsc{Geant4}} with 
...
\begin{figure*}
    ...
    \caption{A comparison of the results of MUSIC, \pascaledit{\textsc{Geant4}}, and PROPOSAL is presented for $\num{e6}$ muons propagated with 
    ...
    Van Ginneken (vG) or as in \pascaledit{\textsc{Geant4}}. Both \pascalreplace{scattering methods}{multiple scattering by Molière (MSM) and Highland (MSH)} are checked. 
    ...
    on the scattering method. \pascalreplace{Molière scattering}{MSM} leads to larger distances.
    In the zoomed-in figure, the region around the mode of the distributions is presented. 
    The mode of \pascaledit{\textsc{Geant4}} is shifted to larger deflections.
    Detailed information are given in 
    Table~\ref{tab:compare_MUSIC}. The results for MUSIC and \pascaledit{\textsc{Geant4}} are taken from 
...
\begin{table*}
    ...
    The largest deflection and displacement is observed in the tool \pascaledit{\textsc{Geant4}}, which has the lowest mean survived energy. The lower the energy, the larger the deflection.}
...
by Molière scattering in \pascaledit{\textsc{Geant4}}. Hence, the lower scattering in PROPOSAL leads 
...
\pascaledit{Even though in the logarithmic figure the simulation date agree well 
...
\begin{figure*}
    ...
    Measured data of Attwood et al. and simulation data of \pascaledit{\textsc{Geant4}} are taken from Ref.~\cite{attwood_2006}.    
    The figure presents 
    ...
    stochastic deflections and Molière scattering \pascaledit{(MSM)}, the orange 
    points
    ...
    \SI{0}{\degree}$ \pascaledit{and at $\theta_{\mathrm{y}} \approx 
    \SI{3}{\degree}$}. At deflections $\SI{2}{\degree} < \theta_{\mathrm{y}} < \SI{5}{\degree}$, 
    the result seems to be more accurate than \pascaledit{\textsc{Geant4}}'s. The consideration of the stochastic deflections shows no significant influence.}
...
\pascaledit{Also simulations with \textsc{Geant4} $\text{v}11.0.3$ using the 
...
\begin{figure*}
    \centering 
    % \resizebox{0.48\textwidth}{!}{%
    \includegraphics{figures/akimenko_comparison_moliere_E7301MeV_final_multi_mean_deg_ratio_G4_v1_final.pdf}
    % }
    \caption{A comparison between two PROPOSAL \pascalreplace{settings}{simulations}
    ...
    \pascaledit{Deflections are simulated with \textsc{Geant4} $\text{v}11.0.3$ as well.}
    ...
    $v_{\mathrm{cut}} = \num{e-5}$. The blue points present the mean 
    \pascaledit{of the simulation} considering stochastic deflections and Molière scattering \pascaledit{(MSM)}, 
    the orange points
    present the mean \pascaledit{of the simulation} taking into account only 
...
\pascaledit{This energy range covers the muon energies typically measured in 
...
\pascaledit{The accumulated deflections and also the propagated distances 
...
\pascaledit{This equation is valid for muon energies between 
...
\begin{figure*}
    ...
    \pascaledit{The exact values and the propagated distances are presented in Table~\ref{tab:final_values}.}}
    \label{fig:fit_median}
\end{figure*}

\subsection{Relevance for Muography}
\pascaledit{Muography is a technique to study the inside of structures with a 
wide field of applications such as the monitoring of volcanic activity an many 
...
of muons can be a limited factor for the angular resolution in 
...
\pascaledit{\textsc{Geant4}} and they are in good agreement. In low-$Z$ materials, the region of outlier 
deflections fits the measured data better with PROPOSAL, than \pascaledit{\textsc{Geant4}}. 
...
energies. 
\pascaledit{In the data--MC ratio deviations up 
...
However, since individual deflections occur in several orders of magnitude, 
the results can still be considered sufficiently accurate.}
...
\begin{table*}
    \centering
    \caption{\pascaledit{The median values for the accumulated deflection $\theta_{\mathrm{acc}}$ in degree and the propagated distances 
