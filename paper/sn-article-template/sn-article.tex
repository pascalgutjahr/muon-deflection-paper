...
of the propagation tools MUSIC and \alexreplace{GEANT4}{Geant4} and all deflections are in good 
agreement. 
}
...
by large neutrino telescopes \alexreplace{like}{such as} IceCube \cite{IceCube_Instrumentation} or 
...
best for muons, since they can travel up to a few \alexreplace{kilo meters}{kilometers} due to their 
large mass, instead of the much lighter \alexreplace{electrons and taus -}{electrons, and taus} which decay almost 
...
In general, muons \alexreplace{do}{undergo} up to several thousand interactions along their propagation, depending 
...
\alexreplace{For this purpose, the lepton propagation 
...
The accumulated deflection is analysed and compared to the propagation codes
...
as bremsstrahlung \cite{KKP_1995, Karnaukhov_1999}, \alexcomment{Vgl. Telegram, Karnaukhov hat hier nichts verloren} photonuclear interaction \cite{Abramowicz_1997} with 
...
\alexcomment{Diese Erklärung der kontinuierlichen Verluste ist unverständlich, wenn man es nicht sowieso schon weiß.} The 
...
\alexcomment{Die stochastischen Ablenkungen sind neu und bislang unveröffentlicht,
du solltest vllt. einen Verweis auf einen Artikel ``in preparation'' über PROPOSAL einfügen,
bspw. als ``Alameddine et al., in preparation''.}
...
For this \alexedit{purpose} muons are propagated from $E_{\text{i}} = \SI{1}{\peta\electronvolt}$ to $E_{\text{f,\,min}} = \SI{1}{\tera\electronvolt}$.
...
\begin{table}
    ...
    central content levels. \alexcomment{Die Tabelle wäre m.\,E. einfacher zu lesen, wenn alles in der gleichen Einheit wäre, also \num{e-5} Grad.}}
...
is lower \alexedit{than} $\sim\SI{1}{\degree}$. Since these deflections accumulate along the 
...
angular reconstruction accuracy. \alexcomment{Diesen Satz verstehe ich nicht.}

\alexreplace{At first}{First}, the deflections in PROPOSAL are compared to 
the tools MUSIC \cite{MUSIC,comparison_MUSIC_GEANT4_2009} and \alexreplace{GEANT4}{Geant4} \cite{GEANT4}.
...
available. For these studies, \alexreplace{it is chosen}{the chosen cross section parametrizations are for} bremsstrahlung \cite{Bremsstrahlung_KKP}, 
...
\alexreplace{which deflections are}{with stochastic deflections as} parametrized by \cite{Van_Ginneken}. 
Also, ionization is treated as a stochastic process with the Bethe-Bloch 
formula including knock-on electron production. As \alexedit{multiple} scattering, the gaussian 
approximation \cite{HIGHLAND_1975} is set. 
\alexreplace{GEANT4}{Geant4} is another common toolkit to simulate the passage of particles through 
...
displacement is dominated by \alexreplace{GEANT4}{Geant4} and PROPOSAL with Molière scattering, which 
...
Detailed information are given in Table~\ref{tab:compare_MUSIC}. In \alexreplace{GEANT4}{Geant4}, the 
largest deflections \alexreplace{result with}{are} $\overline{\theta} = \SI{0.27}{\degree}$ 
and $\overline{x} = \SI{3.3}{\meter}$. The lowest values $\overline{\theta} = \SI{0.22}{\degree}$ and 
$\overline{x} = \SI{2.6}{\meter}$ \alexreplace{result}{are obtained} in MUSIC. The results of PROPOSAL lay between 
these two tools for all four settings which approves the 
correctness of PROPOSAL. \alexcomment{Inwiefern ``approves the correctness''?
Das zeigt nur Konsistenz zwischen verschiedenen Codes.}
...
\begin{figure}
    ...
    Measured data of Attwood and simulation data of \alexreplace{GEANT4}{Geant4} are taken from \cite{attwood_2006}. The figure presents 
    ...
    is underestimating, but at larger deflections the result seems to be more accurate than \alexreplace{GEANT4}{Geant4}'s.}
...
\begin{figure}
    ...
    \caption{A comparison of the results of MUSIC, \alexreplace{GEANT4}{Geant4} and PROPOSAL is presented for $\num{1000000}$ negative charged muons propagated with 
    ...
    Van Ginneken (vG) and \alexreplace{GEANT4}{as in Geant4} and both scattering methods are checked. 
    ...
    Table~\ref{tab:compare_MUSIC}. The results for MUSIC and \alexreplace{GEANT4}{Geant4} are taken from 
...
\begin{table}
    ...
    The largest deflection and displacement \alexreplace{result}{is observed} in the tool \alexreplace{GEANT4}{Geant4}, which has the lowest mean survived energy. The lower the energy, the larger the deflection.}
...
\alexcomment{Was meinst du hier mit ``current analyses''?}
...
the muon energy is done using the \alexedit{average} energy transfer of the neutrino to 
...
At first, \alexreplace{the recently into PROPOSAL implemented stochastic deflection}
{stochastic deflection recently implemented into PROPOSAL} is 
...
The outcome is fit by a polynom\alexedit{ial} and can be used for 
