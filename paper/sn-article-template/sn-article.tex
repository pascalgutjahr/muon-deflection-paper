...
muon reconstructions using PROPOSAL}{Simulation of theoretical deflection uncertainties on directional reconstrcution of muons using PROPOSAL}}
...
\abstract{
To estimate the uncertainty in the reconstruction of the muon direction, the 
deflection of muons along a propagation path in \pascalreplace{ice and water}{dense media} is simulated with the 
tool PROPOSAL, which accounts for multiple scattering and deflection on stochastic interactions. 
The deflection angle per muon interaction $\theta$ extends over several 
orders of magnitude with $\theta \in [\SI{e-9}{\degree},\, \SI{1}{\degree}]$ 
for muon energies 
$\SI{1}{\tera\electronvolt} < E_{\mu} < \SI{1}{\peta\electronvolt}$. 
\pascaldelete{and is negligible for the directional reconstruction.} \pascaledit{The deflection increases as 
the energy decreases and it depends on the energy loss.} The median accumulated 
deflection of a muon with a final energy $E_{\mathrm{f}} = \SI{500}{\giga\electronvolt}$ 
...
present neutrino detectors. The accumulated deflection angle can be parametrized in 
dependence of the final muon energies independent of the initial 
muon energy for the energy range studied here. Furthermore, \pascalreplace{the deflections are compared to the results 
of the propagation tools MUSIC and Geant4 and all deflections are in good 
agreement.}{an agreement is obtained in comparison with the simulation tools MUSIC and Geant4.} 
}

\keywords{neutrino astronomy, \pascalreplace{point-source analysis}{neutrino source search}, angular resolution}
...
to estimate the deflection for \pascaledit{neutrino telescopes such as} IceCube and KM3NeT/ARCA. All relevant muon interaction types 
...
ionization described by the Bethe-Bloch formula with corrections for muons \cite{Rossi} and the decay are provided by PROPOSAL. The interaction processes are sampled by their cross section. Since \pascalreplace{bremsstrahlung interactions can be 
arbitrarily small due to a massless exchange particle, the photon,}{interactions can be arbitrarily small with the massless photon as secondary particle,} an energy cut is introduced to avoid an infinite number of bremsstrahlung interactions 
and furthermore to \pascalreplace{speed up the propagation process}{increase the runtime performance}. 
...
\pascalreplace{A sampled energy loss 
$E_{\text{loss}} < E_{\text{loss,\,min}}$ builds an integrated part of a 
stochastic energy loss referred to as continuous energy loss.}{By the introduction of 
this energy cut, in the propagation the next significant energy loss with $E_{\mathrm{loss}} \geq E_{\text{loss,min}}$ 
...
% An energy loss $E_{\mathrm{loss}} < E_{\text{loss,min}}$ forms an integrated 
% part of the stochastic losses and is denoted as continuous energy loss.
The methodical uncertainties are small 
for a relative energy cut $\texttt{v\_cut}\ll 1$, which increases \pascalreplace{the 
CPU runtime}{the runtime}.
The 
...
\pascaledit{Since muons are unstable, a decay leads to a premature 
...
\pascalreplace{The deflection angle}{The orientation of the deflection} in the plane perpendicular to the muon direction is 
...
A publication \pascalreplace{as Alameddine et al. is in preparation.}{describing the 
...
\begin{figure}
    ...
    \pascaledit{The stochastic interactions 
    are stated as Bremsstrahlung (Brems), photonuclear interaction (Nuclint), electron pair produciton (Epair) and ionization (Ioniz).}
    Two simulations 
    are done to check both multiple scattering methods Molière and Highland. \pascalreplace{The total distribution includes all stochastic processes and Molière's.} 
...
angular reconstruction accuracy.}{This angle limits the angular resolution 
...
\pascalreplace{a medium with several possibilities and a focus on high-energy 
particle accelerators}{matter. The simulation is very precise and especially 
...
the two multiple scattering methods and the different stochastic deflection parametrizations.} The deflection angles are 
...
displacement, since in photonuclear interaction, the sampling from the root mean squared angle in the 
...
\pascalcomment{Den Vgl. werde ich Montag noch durchführen.}
\begin{figure}
    \centering 
...
\begin{figure}
    \centering 
    \includegraphics[width=0.98\textwidth]{../../deflection/plots/FINAL/attwood_comparison_moliere_E199MeV_final.pdf}
    ...
    the normalized counts in dependence of the projected scattering angle $\theta_{\mathrm{y}}$ in radians. \pascaledit{At small angles, deflections are  
    are underestimated in PROPOSAL, except at $\theta_{\mathrm{y}} \approx 0$.} At larger deflections, the result seems to be more accurate than Geant4's.
    \pascaledit{There is no significant difference between the two energy cuts.}}
...
\begin{table}
    \small
    \centering
    \caption{The survival probability $p_{\text{s}}$ \pascalreplace{- otherwise the muon decays before}{that depends on the muon decay}, the mean survived muon 
...
For \pascalreplace{current analyses}{neutrino source searches (based on entering muons)}, it is important to study the impact of the muon 
deflection on the angular resolution to estimate \pascalreplace{an uncertainty on the reconstruction.}{whether or not this needs to be taken into account as an additional source of uncertainty.}
...
in \cite{KM3NeT_Resolution2016, KM3NeT_Resolution2021}. Hence, a \pascalreplace{rescale to 
the muon energy is done}{a rescaling to the muon energy is applied using the} using the average energy transfer of the neutrino to 
the nucleus \cite{GANDHI199681}. This shifts both the curves to lower energies. 
Since all of these simulations are done 
in ice, the same simulations are done in water \pascaledit{to compare the results for 
water-based experiments}. The deviations of the medians
are less than $\SI{1}{\percent}$ for all energies.

\pascalreplace{Furthermore, in Figure~\ref{fig:fit_median} there are only $12$ medians visible, 
instead of $36$ which is the total amount of all simulations. This result points 
out}{Note that the distribution of deflection angles at a given final energy $E_{\mathrm{f}}$ in Figure~\ref{fig:fit_median} overlap for differing initial energies. This result indicates} that the total deflection of a muon 
primarily depends on the final muon energy.
The initial muon energy is \pascalreplace{negligible}{irrelevant}. 
Hence, the reconstructed muon 
energy in a detector can be used to estimate \pascalreplace{a theoretical}{the} deflection. For this 
...
\end{align}
\pascaledit{In general, the function $f(x)$ in Eq.~\eqref{eqn:fit_median} describes the median 
...
\begin{figure}
    ...
    that the true final particle energy $E_{\text{f}}$ is \pascalreplace{maximum}{at most} 
...
Geant4 and \pascalreplace{all}{they} are in good agreement. In low-$Z$ materials, the region of outlier 
deflections \pascalreplace{is described}{fits the measured data} better with PROPOSAL, than Geant4.
...
\pascaledit{
We would like to thank you for the cooperation at our chair at the TU Dortmund
University and the opportunity to advance the research.We also acknowledge the
financial support from the SFB $876$, the SFB $1491$ 
and the DFG project SA~$3867/$2-1.}
